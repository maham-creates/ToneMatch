...
const ProductCard = memo(({
    ...
    <div className="glass p-4 text-left hover:bg-white/10 transition-all group w-full flex flex-col gap-3">
        <button
            onClick={() => onClick(product, category)}
            className="flex items-center gap-3 w-full text-left"
        >
            <div
                className="w-12 h-12 rounded-lg border-2 border-white/20 group-hover:scale-110 transition-transform shrink-0"
            ...
            <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start">
                    ...
                    {product.price && (
                        <span className="text-xs font-mono text-green-300 ml-2">${product.price.toFixed(2)}</span>
                    )}
                </div>
                ...
                <div className="flex justify-between items-center mt-1">
                    ...
                    {product.retailer && (
                        <span className="text-[10px] text-gray-500">{product.retailer}</span>
                    )}
                </div>
            </div>
        </button>
        ...
        {product.url && (
            <a
                ...
                className="w-full text-center text-xs bg-white/5 hover:bg-white/10 border border-white/10 py-1.5 rounded transition-colors flex items-center justify-center gap-2 group-hover:border-purple-500/30"
            >
                <span>Shop Now</span>
